...
from study_buddy.v2.data import create_db_connection
from study_buddy.v2.models import create_embeddings
...
class MemoryManager:
    ...
    def retrieve_memories(self, query: str, user_id: str, k: int = 5) -> List[Memory]:
        ...
        docs = self.vectorstore.similarity_search(query, k=k, filter=filter_function)
        return [Memory.from_document(doc) for doc in docs]

    def find_all_memories(self, user_id: str) -> List[Memory]:
        cursor = self.connection.cursor()
        cursor.execute(FIND_MEMORIES_QUERY, (user_id,))
        rows = cursor.fetchall()

        memories = []

        for row in rows:
            doc = Document(page_content=row[0], metadata=json.loads(row[1]))
            memories.append(Memory.from_document(doc))
